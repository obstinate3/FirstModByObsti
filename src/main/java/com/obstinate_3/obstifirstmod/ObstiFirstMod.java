...
import com.obstinate_3.obstifirstmod.recipe.CraftingRecipe;
import com.obstinate_3.obstifirstmod.recipe.ModRecipe;
...
import com.obstinate_3.obstifirstmod.tileentity.ModTile;
...
public class ObstiFirstMod
{
    ...
    private void registerTileEntity(RegistryEvent.Register<TileEntityType<?>> event) {
        event.getRegistry().register(TileEntityType.Builder.create(FirstBlockTile::new, ModTile.FIRST_BLOCK).build(null).setRegistryName("first_block"));
    }
    ...
    private void registerRecipes(final RegistryEvent.Register<IRecipeSerializer<?>> event) {
        ModRecipe.init();
        IForgeRegistry<IRecipeSerializer<?>> registry = event.getRegistry();

        registry.register(CraftingRecipe.SERIALIZER);
    }
